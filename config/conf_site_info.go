...
type HomeLayoutOrders struct {
	Hot     int `yaml:"hot" json:"hot"`
	Level   int `yaml:"level" json:"level"`
	Profile int `yaml:"profile" json:"profile"`
}

type HomeLayout struct {
	PageSize    int              `yaml:"page_size" json:"page_size"`
	HotCount    int              `yaml:"hot_count" json:"hot_count"`
	LevelCount  int              `yaml:"level_count" json:"level_count"`
	AdvertCount int              `yaml:"advert_count" json:"advert_count"`
	Orders      HomeLayoutOrders `yaml:"orders" json:"orders"`
}

// FriendLink 友情链接配置（作为 site_info 的一部分，随 settings.yaml 一起读写）。
type FriendLink struct {
	Title  string `yaml:"title" json:"title"`     // 展示名称
	Href   string `yaml:"href" json:"href"`       // 跳转链接
	Icon   string `yaml:"icon" json:"icon"`       // 图标（可选，支持 uploads 或完整 URL）
	Desc   string `yaml:"desc" json:"desc"`       // 描述（可选）
	IsShow bool   `yaml:"is_show" json:"is_show"` // 是否展示
}
...
type SiteInfo struct {
	...
	HomeLayout  HomeLayout   `yaml:"home_layout" json:"home_layout"`                                   // 首页布局配置
	FriendLinks []FriendLink `yaml:"friend_links" json:"friend_links"`                                 // 友情链接
